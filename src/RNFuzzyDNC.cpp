...
RNFuzzyDNC::RNFuzzyDNC(){
	...
	distObs->addTerm(new fl::Triangle("close", -std::numeric_limits<double>::infinity(), 0, 1));
	distObs->addTerm(new fl::Triangle("inzone", 0, 1, 2.25));
	distObs->addTerm(new fl::Triangle("far", 1, 2.25, std::numeric_limits<double>::infinity()));
	...
	factor->addTerm(new fl::Triangle("half", 0.4, 0.5, 0.6));
...
void RNFuzzyDNC::getSystemInput(double distance, double* factor){
...
int RNFuzzyDNC::SectorDifference(int alfa, int beta){
...
float RNFuzzyDNC::AngleDifference(float alfa, float beta, int* giro){
	float delta;
...
float RNFuzzyDNC::AngleDifferenceCW(float alfa, float beta){
	float delta;
	float delta1 = abs(alfa-beta);
	float delta2 = 720 - delta1;
...
float RNFuzzyDNC::AngleDifferenceCCW(float alfa, float beta){
	float delta;
	float delta1 = abs(alfa-beta);
	float delta2 = 720 - delta1;
...
float RNFuzzyDNC::AngleMean(float alfa, float beta){
...
float RNFuzzyDNC::LimitAngle(float angle){
...
float RNFuzzyDNC::getDeltaTheta(LaserScan* laserData, ArPose* currPose, ArPose* goalPose){
