    ...
    if(sickLaser == NULL){
      ...
      for(int it = 0; it != currentReadings->size(); it++){
        ...
        if(publish_laser){
          ...
          laserData.intensities.push_back(currentReadings->at(it).getExtraInt());
        }
      }
...
void RosAriaNode::cmd_goto_cb( const geometry_msgs::Pose2DConstPtr &msg){
  veltime = ros::Time::now();
  ROS_INFO( "new pose: [%0.2f mm, %0.2f mm, %0.2f rad](%0.3f)", msg->x*1e3, msg->y*1e3, msg->theta, veltime.toSec() );
  ArPose newPose(msg->x *1000, msg->y * 1000);
  newPose.setThRad(msg->theta);
  robot->lock();
