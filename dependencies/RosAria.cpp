...
class RosAriaNode
{
  ...
  protected:
    ...
    ros::Publisher laser_pointcloud_pub;
...
void RosAriaNode::laserConnectCb()
{
	publish_laser = (laser_pub.getNumSubscribers() > 0);
	publish_laser_pointcloud = (laser_pointcloud_pub.getNumSubscribers() > 0);
}

RosAriaNode::RosAriaNode(ros::NodeHandle nh) : 
  myPublishCB(this, &RosAriaNode::publish), serial_port(""), serial_baud(0),
  sonar_enabled(false), publish_sonar(false), publish_sonar_pointcloud2(false), publish_laser(false), publish_laser_pointcloud(false)
{
  ...
  laser_pub = n.advertise<sensor_msgs::LaserScan>("laser", 100,
      boost::bind(&RosAriaNode::laserConnectCb, this),
      boost::bind(&RosAriaNode::laserConnectCb, this));
  laser_pointcloud_pub = n.advertise<sensor_msgs::PointCloud>("laser_pointcloud", 100,
      boost::bind(&RosAriaNode::laserConnectCb, this),
      boost::bind(&RosAriaNode::laserConnectCb, this));
  ...
  cmd_goto_sub = n.subscribe( "cmd_goto", 1, (boost::function <void(const geometry_msgs::Pose2DConstPtr&)>)
    boost::bind(&RosAriaNode::cmd_goto_cb, this, _1 ));
...
int RosAriaNode::Setup()
{
  ...
  argparser->addDefaultArgument("-connectLaser -lp /dev/ttyS2 -ld 180 -li half -lmr 30000 -lf true"); //-lp /dev/ttyS2 port to connect
  ...
  if(!laserConnector->connectLasers(false, false, true)){
	ROS_DEBUG("Could not connect to configured lasers.");
  }
  
  ROS_INFO("Connected to lasers.");
  ArUtil::sleep(500);
  laser = robot->findLaser(1);
  
  if(!laser){
	ROS_INFO("Error. Not Connected to any laser.");
  }
...
void RosAriaNode::publish()
{
  ...
  // Publish Laser Information
  if(publish_laser || publish_laser_pointcloud){
	  sensor_msgs::LaserScan laserData;
	  sensor_msgs::PointCloud laserDataPointCloud;
	  ArSick* sickLaser = (ArSick*)laser;
	  if(sickLaser == NULL){
		ROS_INFO("Sick is NULL");
	  }
	  laserData.header.stamp = position.header.stamp;
	  laserData.header.frame_id = frame_id_laser;
	  
	  laserDataPointCloud.header.stamp = position.header.stamp;
	  laserDataPointCloud.header.frame_id = frame_id_laser;
		if(publish_laser){
			laserData.angle_min = -M_PI / 2.0;
			laserData.angle_max = M_PI / 2.0;
			double increm = sickLaser->getIncrement() == ArSick::INCREMENT_ONE? 1 : 0.5;
			laserData.angle_increment = increm * M_PI / 180.0;
			
			laserData.range_min = ((double)sickLaser->getMinRange()) / 1000.0;
			laserData.range_max = ((double)sickLaser->getAbsoluteMaxRange()) / 1000.0;
		}
	  int laserIndex = 1;
	  laser->lockDevice();
	  ...
	  if(publish_laser){
		laser_pub.publish(laserData);
	  }
	  if(publish_laser_pointcloud){
		laser_pointcloud_pub.publish(laserDataPointCloud);
	  }
  }
}
...
void RosAriaNode::cmd_set_pose( const geometry_msgs::Pose2DConstPtr &msg)
{
  veltime = ros::Time::now();
  ROS_INFO( "new pose: [%0.2f, %0.2f, %0.2f](%0.3f)", msg->x, msg->y, msg->theta, veltime.toSec() );
  ...
  ROS_DEBUG("RosAria: sent pose to aria (time %f): x pose %f mm, y pose %f mm, theta pose %f rad", veltime.toSec(),
    (double) msg->x * 1000, (double) msg->y * 1000, (double) msg->theta);
}

void RosAriaNode::cmd_goto_cb( const geometry_msgs::Pose2DConstPtr &msg)
{
  veltime = ros::Time::now();
  ROS_INFO( "new pose: [%0.2f mm, %0.2f mm, %0.2f rad](%0.3f)", msg->x, msg->y, msg->theta, veltime.toSec() );
  ArPose newPose(msg->x *1000, msg->y * 1000);
  newPose.setThRad(msg->theta);
  robot->lock();
  ...
  if(msg->x == 0.0 && msg->y == 0.0 && msg->theta != 0.0){
    robot->setHeading(msg->theta * 180/M_PI);
  } else {
